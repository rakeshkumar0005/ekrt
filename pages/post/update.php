...
if(isset($_POST['edit'])) {
  ...
  $db->query("UPDATE  post SET title='{$title}',heading='{$heading}',sub_heading='{$sub_heading}', date_time='{$date_time}',image='{$image}',description='{$description}',authors='{$authors}' WHERE id ='{$_GET['id']}'");

      echo "<font color=#0ddbb9>Edited Sucessfully!</font>";
...
<div class="col-md-7 col-lg-10 m-auto">
        <form method="POST">
          <div class="row g-3">
            ...
            <div class="col-12 mt-3">
              <input type="text" class="form-control" name="description" placeholder="Description" value="<?=$enter['description']?>">
            ...
            <select class="form-control" type="text" name="authors">
            <option>Select Author</option>
            <?php
              while($enter=mysqli_fetch_array($query)) 
                
              echo "<option>" .$enter['name']."</option>";
             ?>
